...
export default {
  ...
  created() {
    ...
    console.log(token)
    if (token) {
      console.log("Websocket Config here")
      this.initializeEcho();
    }
  },
  mounted() {
...
window.Echo = new Echo({
  ...
  try {
    ...
    //       // Convert 'attachment' path to 'attachment_url' for immediate display
    ...
    //       const messageReceiverId = Number(message.receiver_id);
    //       const selectedUserId = Number(this.selectedUser?.id);

    //       const isChatOpen =
    //         selectedUserId &&
    //         (selectedUserId === messageSenderId || selectedUserId === messageReceiverId);

    //       if (isChatOpen) {
    //         console.log("✅ Adding to selected chat:", message);
    //         if (!Array.isArray(this.selectedUser.messages)) {
    //           this.selectedUser.messages = [];
    //         }
    //         this.selectedUser.messages.push(message);
    //         this.selectedUser.messages = [...this.selectedUser.messages]; // force reactivity update
    //         this.$nextTick(() => this.scrollToBottom());
