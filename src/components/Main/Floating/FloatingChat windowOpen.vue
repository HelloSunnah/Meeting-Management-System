<template>
  <div :class="themeClass">
    <!-- Chat Button -->
   <button ref="chatButton" @click="toggleChatWindow"
  ...
  <span v-if="totalUnreadMessages > 0" 
    class="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full px-2 py-0.5 shadow">
    {{ totalUnreadMessages }}
  </span>
        ...
        <div class="flex flex-col flex-grow overflow-auto no-scrollbar bg-gray-100">
          ...
          <div class="flex items-center">
            ...
            <div class="flex flex-col">
              <p class="font-semibold text-gray-900 text-md">{{ user.name }}</p>
              <p class="text-sm text-gray-600 truncate max-w-[200px]">{{ user.department_name || 'No bio available' }}</p>
            </div>
          </div>

          <!-- 🔴 Unread Message Count -->
          <span v-if="unreadMessages[user.id] > 0"
                class="bg-red-500 text-white text-xs font-bold rounded-full px-2 py-0.5 shadow ml-4">
...
export default {
  ...
  computed: {
    ...
    totalUnreadMessages() {
      return Object.values(this.unreadMessages).reduce((sum, count) => sum + count, 0);
    }
  },
  async created() {
    try {
  ...
  methods: {
    ...
    openChat(user) {
      ...
      this.unreadMessages[user.id] = 0; // ✅ Reset unread
    },
  ...
  listenForNewMessages(user) {
      ...
      window.Echo.private(chatChannel)
        .listen(".MessageSent", (data) => {
          ...
          // This block is only for the RECEIVER side
          const senderUser = this.users.find(u => u.id === senderId);
