  ...
  >
    ...
    <div class="p-6 flex flex-col h-full">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-semibold text-gray-800 leading-tight">{{ poll.polling_title }}</h2>
        <span class="px-4 py-2 text-xs rounded-full font-semibold"
          :class="poll.type === 1 ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'">
          {{ pollTypeLabel }}
        </span>
      </div>
      ...
      <div class="text-gray-500 text-xs mb-4 space-y-1">
        <p><i class="fas fa-calendar-alt mr-1 text-green-500"></i>Start: {{ formatDate(poll.start_time) }}</p>
        <p><i class="fas fa-hourglass-end mr-1 text-red-400"></i>End: {{ formatDate(poll.end_time) }}</p>
        <p><i class="fas fa-users mr-1 text-indigo-400"></i>Participants: {{ poll.total_votes }}</p>
      </div>
      ...
      <div>
        ...
        <ul class="space-y-2">
          <li v-for="option in poll.results" :key="option.option_id">
            <button @click="$emit('open-modal', poll, option)" class="w-full text-left text-gray-700 hover:text-blue-600 font-medium">
        ...
        <div class="mt-6">
          <PollPieChart :votes="poll.results.map(opt => ({ option: opt.option_name, count: opt.votes }))" />
        </div>

        <button
          v-if="isAdmin"
          class="mt-4 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded"
          @click="$emit('toggle-publish', poll)"
        >
